# Tracker index (regenerated 2026-09-09T23:36:53Z — do not edit)

## open
- BUG-0154 [P3][bug][patch] A source whose registry NAME is blank reaches the screen as a blank evidence cell and a link with nothing to read — the third rendering of one row's absence  (scope:src/lib/sources/names.ts,src/lib/db/review-item.ts,src/components/review/evidence-cells.tsx,src/components/claims/claim-list.tsx,tests/offline/review-item/page.test.ts,tests/offline/sources/names.test.ts from:BUG-0152)
...
## claimed
- BUG-0153 [P3][bug][M2] /cycles spells a raw ?source= into the runs window line as bare text, so a bidi control in the URL reverses 87 characters of the app's own prose  (scope:src/components/cycles/adapter-runs.tsx,src/app/cycles/page.tsx,tests/offline/cycles/page.test.ts @builder-228 from:BUG-0147)
...
Totals — blocked:1, claimed:1, done:112, open:3, wont_fix:1. Archived: 124.
