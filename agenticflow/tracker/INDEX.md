# Tracker index (regenerated 2026-09-15T08:01:14Z — do not edit)
...
## built
- BUG-0234 [P2][bug][patch] A host answer is admitted leg by leg, so three legs still admit an answer that is not a PostgREST answer  (scope:src/lib/db/result.ts,src/lib/db/schema.ts,tests/offline/db/result.test.ts,tests/offline/db/schema.test.ts @builder-334 from:BUG-0229)
...
- TASK-0084 [P2][task][patch] HOLD until Ben answers — acceptance test 6: every §7 action proven end to end on staging  (scope:tests/live/settle.live.test.ts,tests/live/sweep.ts,tests/live/staging-target.ts)
- TASK-0085 [P2][task][patch] HOLD until Ben answers — acceptance test 7's override half: an events/venues edit lands as an admin_locked observation with its override verdict row  (deps:TASK-0084(unmet:1) scope:tests/live/edit.live.test.ts,tests/live/sweep.ts)
- TASK-0086 [P2][task][patch] HOLD until Ben answers — acceptance test 8: a reference-field override produces venue_id / event_performers rows, never text  (deps:TASK-0085(unmet:1) scope:tests/live/edit.live.test.ts,tests/live/review-item.live.test.ts,tests/live/sweep.ts)

Totals — blocked:4, built:1, done:25, open:4. Archived: 328.
